...
- `apps/api/src/db/schema/` にドメインテーブルを Drizzle ORM で定義する
  ...
  - `evolution_chains`（進化系統のグルーピング単位。属性は将来追加できるよう `id` のみで開始）
  - `species` / `species_names`（ポケモン種族とその多言語名。`national_dex_number` を保持し、`evolution_chain_id` で同系統をグルーピング）
  ...
  - `form_sprites`（フォーム × 性別 × 種別 × URL。`(form_id, gender, kind)` で一意。`url` は Supabase Storage の相対パスを格納）
  - `pokedex_entries`（図鑑ごとの図鑑番号。`(pokedex_id, pokedex_number)` で一意、`(pokedex_id, species_id)` で同一図鑑への species 重複登録を防ぐ）
...
- 最小シードデータ（JSON ファイル）と、JSON を読み込んで DB にインサートする `pnpm --filter @pokedex/api seed` スクリプトを追加する
  - 最初のシード対象は **代表的なフォーム 100+ をカバー** する（ロトムのヒート、アルセウスの 18 プレート、シルヴァディのメモリー、オーガポンの 4 仮面、アンノーンの 28 文字、ジガルデの 3 形態、ネクロズマの 4 形態、ザシアン・ザマゼンタの剣の王/楯の王、ゲンシグラードン/カイオーガ、コスプレピカ・キャップピカ、テラパゴスのステラ形態 など）
...
- ドメイン不変条件チェック（**Invariant Tests**）を vitest で実装する
  ...
  - `species_evolutions` の両端が同じ `evolution_chain_id` を持つこと
