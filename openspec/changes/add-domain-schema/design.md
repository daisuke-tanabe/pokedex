...
- `FormCategory` のハイフン (`mega-x` / `mega-y`) と `SpriteKind` のアンダースコア (`back_shiny`) の区切り文字不整合 → **PokeAPI 規約に合わせる方針**で揃える。PokeAPI の `pokemon-form` リソースは `charizard-mega-x` のようにフォーム名でハイフンを使い、スプライト種別は `back_shiny` / `front_default` のようにアンダースコアを使うため、本 contracts もそれに準拠する。同一パッケージ内では一見不統一に見えるが、外部 API との互換を取るための意図的な選択である
