...
- `species.evolution_chain_id` は `evolution_chains.id` を参照する FK。NOT NULL とし、進化しない種族にも独自のチェーン行を割り当てる（"自分自身しか含まないチェーン"）
...
- `form_sprites` は `(form_id, gender, kind)` を UNIQUE にする。`gender` は `'male'` / `'female'` / `'unknown'`、`kind` は `'default'` / `'shiny'` / `'back'` / `'back_shiny'`
- `pokedexes.region_id` は `regions.id` を参照する FK（NULL 許容: `national` 図鑑など特定地方に属さない図鑑がある）
- `pokedex_entries` は `(pokedex_id, pokedex_number)` を UNIQUE、`(pokedex_id, species_id)` を UNIQUE にする。同じ species が複数の `pokedex_id` に紐づくのは正常（national + paldea 両方に登録される）
...
- `species.evolution_chain_id`: NOT NULL FK to `evolution_chains.id`
...
- `evolution_chains`: `id` のみ（属性は将来追加）。同じ進化系統に属する species 全員に同じ `evolution_chain_id` を割り当てる
- `species.evolution_chain_id`: NOT NULL FK。**進化しない種族にも独自のチェーンを 1 行作って割り当てる**
...
- 進化しない種族にもチェーンを割り当てることで `species.evolution_chain_id` を NOT NULL にでき、クエリ時に NULL ハンドリングが不要
...
- **[リスク] 進化しない種族にも `evolution_chain_id` を必須にすることでチェーン行が膨れる**: 1 種族 1 チェーンが大半になる → Mitigation: 単一行でもチェーンを持たせる方が `NULL` ハンドリングが消える利点が上回る。`evolution_chains` は `id` のみのテーブルなので行数増加によるストレージ影響は無視できる
...
- 「進化しない species」に独自の `evolution_chain_id` を割り当てる運用: シード時に `species.json` の各エントリで `evolution_chain_id` を明示するか、`null` を許して seed 後に補完するか → 本 change では **シード JSON で明示する** 方針（不変条件チェックが単純になる）
