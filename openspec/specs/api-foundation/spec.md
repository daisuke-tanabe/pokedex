...
#### Scenario [integration]: ヘルスチェックが成功エンベロープを返す

- **WHEN** `GET /health` を呼び出す
- **THEN** HTTP ステータス 200 と JSON ボディ `{ "success": true, "data": { "status": "ok" } }` が返る

#### Scenario [integration]: ヘルスチェックは Content-Type が application/json
...
#### Scenario [unit]: db シンボルは複数 import で同一インスタンス
...
#### Scenario [unit]: AppType がパッケージ外から参照可能
...
#### Scenario [unit]: 成功エンベロープに meta を付与できる
