...
#### Scenario [unit]: 6 つのシード JSON ファイルが存在する
...
#### Scenario [unit]: locales.json が valibot スキーマでパースできる

- **WHEN** `locales.json` を読み込み、対応する valibot スキーマで `parse` する
- **THEN** 例外を投げずに同じ構造の値が返る

#### Scenario [unit]: types.json が valibot スキーマでパースできる

- **WHEN** `types.json` を読み込み、対応する valibot スキーマで `parse` する
- **THEN** 例外を投げずに同じ構造の値が返る

#### Scenario [unit]: species.json が valibot スキーマでパースできる

- **WHEN** `species.json` を読み込み、対応する valibot スキーマで `parse` する
- **THEN** 例外を投げずに同じ構造の値が返る

#### Scenario [unit]: forms.json が valibot スキーマでパースできる
...
#### Scenario [unit]: locales.json に ja と en が含まれる

- **WHEN** `locales.json` を読み込み、各エントリの `code` を抽出する
- **THEN** `'ja'` と `'en'` の 2 値が最低含まれる

#### Scenario [unit]: locales.json の code 集合が contracts の Locale と一致する
...
#### Scenario [unit]: forms.json に複数フォームを持つ species が最低 1 種含まれる

- **WHEN** `forms.json` を読み込み、`species_slug` ごとのエントリ数を集計する
- **THEN** エントリ数が 2 件以上の `species_slug` が最低 1 つ存在する

#### Scenario [unit]: forms.json に最低 2 種類の category バリエーションが含まれる
...
#### Scenario [integration]: 途中で挿入が失敗するとトランザクション全体がロールバックされる
...
#### Scenario [integration]: placeholder URL で form_sprites を insert できる

- **WHEN** `(form_id=X, gender='male', kind='default', url='placeholder/pikachu/normal/male/default.png')` を insert する
- **THEN** 成功する

#### Scenario [integration]: 空文字の url は拒否される
...
#### Scenario [integration]: national_dex_number と national pokedex の番号が一致する
...
#### Scenario [integration]: species_evolutions の両端が同じ evolution_chain_id を共有する
...
#### Scenario [integration]: pokedex_entries.form_id が指す form は同じ species に属する
...
#### Scenario [unit]: forms.json の全エントリに isDefault フィールドが存在する

- **WHEN** `forms.json` を読み込み、`formsFileSchema` で valibot パースする
- **THEN** 全エントリで `isDefault` フィールドが boolean として解決される

#### Scenario [unit]: 各 species に isDefault = true のエントリが 1 件だけ存在する
...
#### Scenario [unit]: forms.json の slug が species_slug を prefix として含まない（通常フォームを除く）

- **WHEN** `forms.json` の全エントリで `speciesSlug` と `slug` を比較する
- **THEN** 非通常フォーム (`isDefault = false`) の slug は対応する species_slug を prefix として含まない

#### Scenario [unit]: pokedexes.json の formSlug が forms.json と一致する短縮形である
...
#### Scenario [integration]: DATABASE_URL 未設定で test ファイルがロードに失敗しない
...
#### Scenario [integration]: DATABASE_URL 設定済みで invariant 検証が走る
