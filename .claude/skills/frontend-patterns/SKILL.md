...
- 状態を管理するとき(useState, useReducer, Zustand, Context)
- データ取得を実装するとき(SWR, React Query, server components)
- パフォーマンスを最適化するとき(memoization, virtualization, code splitting)
- フォームを扱うとき(バリデーション、controlled inputs、Zod schema)
- クライアントサイドルーティングとナビゲーションを扱うとき
- アクセシブルでレスポンシブな UI パターンを構築するとき
...
**留意点**: モダンフロントエンドパターンは保守性と高性能な UI を実現する。プロジェクトの複雑さに合わせてパターンを選ぶ。
