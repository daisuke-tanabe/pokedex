...
PostgreSQL のベストプラクティスのクイックリファレンス。本文の各セクション（インデックス／データ型／一般パターン／アンチパターン検出／設定テンプレート）に従い、深掘りが必要な場合は本書の参考リンクや一次資料（PostgreSQL 公式ドキュメント、Supabase ドキュメント）を直接参照する。
