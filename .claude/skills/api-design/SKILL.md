---
name: api-design
description: リソース命名、ステータスコード、ページネーション、フィルタリング、エラーレスポンス、バージョニング、レート制限を含む本番 API 向けの REST API 設計パターン。
---

# API 設計パターン

一貫性があり、開発者にやさしい REST API を設計するための規約とベストプラクティス。

## 起動タイミング

- 新しい API エンドポイントを設計する
- 既存の API 契約をレビューする
- ページネーション、フィルタリング、ソートを追加する
- API のエラーハンドリングを実装する
- API バージョニング戦略を計画する
- パブリック／パートナー向け API を構築する
...
- [ ] 正しい HTTP メソッドが使用されている（読み取りに GET、作成に POST 等）
- [ ] 適切なステータスコードが返される（すべて 200 ではない）
- [ ] 入力がスキーマで検証されている（Zod、Pydantic、Bean Validation）
- [ ] エラーレスポンスがコードとメッセージを持つ標準フォーマットに従う
