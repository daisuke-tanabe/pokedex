...
- `Investigate`: 深掘りレビュー必要 (CI を fail。誤判定または人間が確認・承認した場合は PR に `deps-review-approved` ラベルを付与することで override 可能)
- `Hold`: マージ保留 (CI を fail。誤判定または人間が承認した場合は `deps-review-approved` ラベルで override 可能)
