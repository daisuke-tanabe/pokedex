...
2. **`verify`** ジョブ (`verify.yml`: typecheck / lint / format) が全 PR で走る。生成される
   `verify / verify` チェックが `main` ブランチ ruleset の **唯一の必須ステータスチェック**。
...
- 必須ステータスチェック: **`verify / verify` のみ** (`deps-review` は必須ではない)
...
マージ**できる (必須の `verify / verify` さえ通っていればよい)。「自動マージされない = 人手判断を
...
| 必須ゲート | GitHub ruleset (`verify / verify` 必須 / 削除・force push 禁止) | (GitHub ruleset / repo 設定) |
