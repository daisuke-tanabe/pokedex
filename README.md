...
2. **`verify`** ジョブ (`verify.yml`: typecheck / lint / format) が全 PR で走る。生成される
   `verify / validate` チェックが `main` ブランチ ruleset の **唯一の必須ステータスチェック**。
...
- 必須ステータスチェック: **`verify / validate` のみ** (`deps-review` は必須ではない)
...
マージ**できる (必須の `verify / validate` さえ通っていればよい)。「自動マージされない = 人手判断を
...
| 必須ゲート | GitHub ruleset (`verify / validate` 必須 / 削除・force push 禁止) | (GitHub ruleset / repo 設定) |
